...
// Transpiler v0.2 — soporta AST con items[], cuerpos de funciones/efectos,
...
export function emitTypeScript(program: Program): string {
  ...
  out.push(`// Generated by IntentLang v0.2`);
  out.push(`// DO NOT EDIT MANUALLY\n`);

  // Prelude mínimo: tipos de utilidades si no se importan fuera.
  // Lo dejamos comentado por si prefieres usar tu runtime local.
...
function emitMatchExpr(m: MatchExpr, isEffect: boolean): string {
  const subject = `_m_${fresh()}`;
  const head = `(() => { const ${subject} = ${emitExpr(m.expr, isEffect)};`;
  const chain = emitCasesAsIfs(subject, m.cases, isEffect, /*returns*/ true);
  return `${head}\n${indent(chain)}\n})()`;
}
