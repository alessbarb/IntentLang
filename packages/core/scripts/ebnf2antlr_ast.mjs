...
function findFirstProdIndex(src) {
  ...
  while (i < n) {
    ...
    // Outside comments/lexer/strings → check if at start of a line we have:  Ident = ...
    if (i === lineStart) {
      let j = i;
      if (/[A-Za-z_]/.test(src[j])) {
        ...
        if (src[k] === "=") return i; // first production
      }
    }
...
function parseDirectives(src, debug, outDir) {
  ...
  const headerRaw = src.slice(0, cut);
  const restRaw = src.slice(cut); // whole grammar body from first prod
...
function buildParser(grammar, parserRules) {
  return `parser grammar ${grammar}Parser;
