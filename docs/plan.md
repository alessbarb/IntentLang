...
export function lex(input: string): Token[] {
  ...
  while (i < input.length) {
    ...
    if (c === "/" && peek2() === "*") {
      advance(2);
      while (i < input.length && !(peek() === "*" && peek2() === "/"))
        advance();
      advance(2);
      continue;
    }
...
export function parse(input: string): Program {
  ...
  const eat = (type?: Token["type"]) => {
    const t = tokens[p++];
    if (type && t.type !== type)
      throw err(`Expected ${type} but got ${t.type}`, t);
    return t;
  };
  const peek = (type: Token["type"]) => cur().type === type;
  const err = (msg: string, t = cur()) =>
    new Error(`${msg} at ${t.line}:${t.column}`);
  ...
  const span = {
    start: { line: 1, column: 1, index: 0 },
      ...
      ? { line: cur().line, column: cur().column, index: cur().index }
  ...
  function parseTypeExpr(): TypeExpr {
    ...
    if (peek("ident") && tokens[p + 1]?.type === "lt")
    ...
    function asCtor(x: TypeExpr): UnionCtor {
      if (x.kind === "LiteralType")
        return {
          kind: "UnionCtor",
          name: {
            kind: "Identifier",
            name: JSON.stringify(x.value),
            span: x.span,
          },
          span: x.span,
        };
      if (
        ...
        return {
          kind: "UnionCtor",
          name: { kind: "Identifier", name: typeToString(x), span: x.span },
          span: x.span,
        };
      ...
      return {
        kind: "UnionCtor",
        name: { kind: "Identifier", name: "Unknown", span: x.span },
        span: x.span,
      };
    }
  }
  ...
  function parseRecordType(): RecordType {
    ...
    while (!peek("rbrace")) {
      ...
      if (peek("kw_where")) {
        eat("kw_where");
        refinement = captureUntil(["comma", "rbrace"]);
      }
      if (peek("comma")) eat("comma");
      fields.push({
        kind: "RecordField",
  ...
  function parseCtorOrBasicOrBrandOrLiteral(): TypeExpr {
    if (peek("string"))
      return { kind: "LiteralType", value: string(), span: pos() };
  ...
  function typeToString(t: TypeExpr): string {
    switch (t.kind) {
      case "BasicType":
        return t.name;
      case "BrandType":
        return `${t.base.name} & Brand<"${t.brand}">`;
      case "LiteralType":
        return JSON.stringify(t.value);
      case "GenericType":
        return `${t.name.name}<${t.params.map(typeToString).join(", ")}>`;
      case "RecordType":
        return `{ ${t.fields.map((f) => `${f.name.name}: ${typeToString(f.type)}`).join("; ")} }`;
      case "UnionType":
        return t.ctors.map((c) => c.name.name).join(" | ");
    }
  }
}
```
...
export function checkCapabilities(program: Program): Diagnostic[] {
  ...
  const declared = new Set(program.uses?.entries.map((u) => u.name.name) ?? []);
...
export function emitTypeScript(program: Program): string {
  const out: string[] = [];
  out.push(`// Generated by IntentLang v.0.3);
